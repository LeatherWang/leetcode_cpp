...
public:
    ...
    unsigned Hash(KeyType k) //【1】hash函数的一种(取模法)
    {
        return k%Hash_length;
    }

    void Collision(int &p, int d) //【2】解决冲突
    {
        p = (p+d)%Hash_length; //采用开放地址法里的线性探测
    }

    bool Search_Hash(KeyType k, int &p) //查找，根据键值查找到地址p
    {
        ...
        /** @attention 注意这里的第一个条件!! 因为存的时候是这样存的，所以如果出现NULLKEY，肯定是到头了*/
        while((elem[p].key != NULLKEY) && (elem[p].key != k))
        {
            c++;
            if(c < Hash_length)
                Collision(p, c); /** @attention 注意这里是怎么解决冲突的，很机制啊，每次都这样*/
            else
                return 0; //表示查找不成功
        }
